 ...
 */
@ConvertAsProperties(
    dtd = "-//org.jfx.browser//StartUpBrowser//EN",
autostore = false)
...
@Messages(
{
	"CTL_StartUpBrowserAction=StartUpBrowser",
	"CTL_StartUpBrowserTopComponent=StartUpBrowser Window",
	"HINT_StartUpBrowserTopComponent=This is a StartUpBrowser window"
})
public final class StartUpBrowserTopComponent extends TopComponent
{
	private Pane browser;
	private static final int PANEL_WIDTH_INT = 675;
	private static final int PANEL_HEIGHT_INT = 400;

	public static StartUpBrowserTopComponent Current;
	...
	private void initAndShowGUI(String _url)
	{
		...
		Platform.runLater(new Runnable()
		{
			...
			public void run()
			{
				...
				m_panel.setScene(new Scene(browser));
			}
		});
	}

	private Pane createBrowser(String url)
	{
		...
		EventHandler<ActionEvent> goAction = new EventHandler<ActionEvent>()
		{
			...
			{
				m_engine.load(locationField.getText().startsWith("http://") ? locationField.getText()
	...
	{
		//close the palette topcomponent if it was instantiated with the configuration
		TopComponent tcPalette = WindowManager.getDefault().findTopComponent("CommonPalette");
		
		if (tcPalette != null) // && tcPalette.isOpened())
			tcPalette.close();

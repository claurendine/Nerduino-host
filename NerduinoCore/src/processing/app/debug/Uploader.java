...
public abstract class Uploader implements MessageConsumer
{
			...
			throws RunnerException
	{
		...
		try
		{
			if (verbose || Preferences.getBoolean("upload.verbose"))
			{
				...
				{
					io.getOut().print(commandArray[i] + " ");
			...
			while (compiling)
			{
				...
				{
					io.getOut().println("Interupted!");					
				}
			}
			if (exception != null)
			{
				io.getOut().println(exception.getMessage());					
		...
		catch(Exception e)
		{
			...
			{
				io.getOut().println("uisp is missing!");					
	...
	public void message(String s)
	{
		...
		if (!Preferences.getBoolean("upload.verbose") && (s.indexOf("Connecting to programmer:") != -1

 ...
 */
public class Sketch extends TreeNode
{
	...
	String m_sketchPath;
	String m_primaryClassName;
	ArduinoSourceEditor m_editor;
	NerduinoBase m_targetNerduino;
	CompileAllCommand m_compileButton;
	boolean m_hasError;
	boolean m_isDirty;
	...
	public Sketch()
	{
		super(new Children.Array(), "Sketch", "/com/nerduino/resources/Sketch16.png");
	}
    ...
    /*
		 ...
		 */
		tempBuildFolder = ArduinoManager.Current.getBuildFolder();
	...
	public synchronized void fireCompileStatusUpdate(boolean compiling, boolean compiled, int percentComplete, String error) 
	{
		StatusUpdateEventClass event = new StatusUpdateEventClass(this);
		
		event.statusType = StatusUpdateEventClass.COMPILE;
		event.pending = compiling;
		event.succeeded = compiled;
	...
	private synchronized void fireDirtyStatusUpdate() 
	{
		StatusUpdateEventClass event = new StatusUpdateEventClass(this);
		
		event.statusType = StatusUpdateEventClass.DIRTY;
		event.pending = true;
	...
	public void cloneSketch(Sketch selectedSketch)
	{
		...
		if (!f.exists())
		{
			...
			// copy the folder to the new project folder
			File[] list = selectedSketch.getFolder().listFiles();

			if (list != null)
			{
				for (File child : list)
				{
					// rename the project pde/ino to the new project name
					String filename = child.getName();
					int index = filename.indexOf(".");

					if (index > 0)
					{
						String rootname = filename.substring(0, index);

						if (rootname.equalsIgnoreCase(oldname))
						{
							filename = newname + filename.substring(index);
						}
					}
					
					newpath = path + "/" + filename;

					copyFile(child.getAbsolutePath(), newpath);
				}
			}
		}
		...
		{
			//Sketch newsketch = new Sketch(newpath);
			SketchManager.Current.addSketch(this);
			this.init(newpath);
			this.select();
		}
	}
    ...
    public TopComponent getTopComponent()
	{
		...
		FileObject multiviewsFolder = FileUtil.getConfigFile("sketchmultiviews");
		FileObject[] kids = multiviewsFolder.getChildren();
		MultiViewDescription[] descriptionArray = new MultiViewDescription[kids.length];
		List<MultiViewDescription> listOfDescs = new ArrayList<MultiViewDescription>();
	...
	{
		m_targetNerduino = nerduino;
	}
	
	public NerduinoBase getTarget()
	{
		return m_targetNerduino;
	}
	...
	public final class CloneNodeAction extends AbstractAction
	{
		...
		{
			node = lookup.lookup(Sketch.class);
		...
		public void actionPerformed(ActionEvent e)
		{
			if (node != null)
			{
				try
				{
					...
					newsketch.setName(node.getName());
					
					newsketch.cloneSketch(node);
					
					newsketch.showTopComponent();
				}
				catch(Exception ex)
				{
				}
	...
	public void delete()
	{
		...
		if (resp == 0)
		{
			...
			SketchManager.Current.removeSketch(this);
		}
	}
	...
	{
		// rename the primary file name to the newname

		String oldpath = ArduinoManager.Current.getArduinoPath() + "/" + oldname;

		// verify that the path exists
		String newpath = ArduinoManager.Current.getArduinoPath() + "/" + newname;
	...
	String getBuildPath()
	{
		return ArduinoManager.Current.getArduinoPath() + "/" + m_sketchPath;
	}
	...
	public void compile()
	{
		try
		{			
			fireCompileStatusUpdate(true, false, 0, "Compiling");
			
			build(m_verbose);
			
			m_hasError = false;
			
			StatusDisplayer.getDefault().setStatusText("Compiling " + this.getName() + " Succeeded!");
		}
		catch(RunnerException ex)
		{
			m_hasError = true;
			fireCompileStatusUpdate(false, false, 0, "Compile failed");
			
			StatusDisplayer.getDefault().setStatusText("Compiling " + this.getName() + " Failed!");
		}
	...
	/**
	 * Build the list of files. <P> Generally this is only done once, rather
	 * than each time a change is made, because otherwise it gets to be a
	 * nightmare to keep track of what files went where, because not all the
	 * data will be saved to disk. <P> This also gets called when the main
	 * sketch file is renamed, because the sketch has to be reloaded from a
	 * different folder. <P> Another exception is when an external editor is in
	 * use, in which case the load happens each time "run" is hit.
	 */
	protected void load()
	{
	...
	protected void sortCode()
	{
		...
		for (int i = 1; i < codeCount; i++)
		{
			...
			if (who != i)
			{  // swap with someone if changes made
	...
	protected void nameCode(String newName)
	{
		...
		if (!validExtension(newExtension))
		{
			Base.showWarning(_("Problem with rename"),
							 I18n.format(
					_("\".{0}\" is not a valid extension."), newExtension), null);
			return;
		}
...
//    	editor.header.rebuild();
	...
	public void handleDeleteCode()
	{
		...
		int result = JOptionPane.showOptionDialog(WindowManager.getDefault().getMainWindow(),
												  prompt,
												  _("Delete"),
												  JOptionPane.YES_NO_OPTION,
...
//		}
	...
	/**
	 * Handles 'Save As' for a sketch. <P> This basically just duplicates the
	 * current sketch folder to a new location, and then calls 'Save'. (needs to
	 * take the current state of the open files and save them to the new
	 * folder.. but not save over the old versions for the old sketch..) <P>
	...
	protected boolean saveAs() throws IOException
	{
		...
		if (isReadOnly() || isUntitled())
		{
			...
			fd.setDirectory(ArduinoManager.Current.getArduinoPath());
		}
		else
		{
			// default to the parent folder of where this was
...
//    	editor.base.rebuildSketchbookMenus();
	...
	/**
	 * Prompt the user for a new file to the sketch, then call the other
	 * addFile() function to actually add it.
	 */
	public void handleAddFile()
	{
		...
		FileDialog fd = new FileDialog(WindowManager.getDefault().getMainWindow(), prompt, FileDialog.LOAD);
		fd.setVisible(true);
...
//		}
	 ...
	 * <p/>
	 * .pde or .java files will be added to the sketch folder. <br/> .jar,
	 * .class, .dll, .jnilib, and .so files will all be added to the "code"
	 * folder. <br/> All other files will be added to the "data" folder.
	 * <p/>
...
//			editor.header.repaint();
//			if (editor.untitled)
//			{  // TODO probably not necessary? problematic?
...
//		}
	...
	*/
	 ...
	 * index. <OL> <LI> store the String for the text of the current file. <LI>
	 * retrieve the String for the text of the new file. <LI> change the text
	 * that's visible in the text area </OL>
	 */
...
//    	editor.header.rebuild();
	...
	/**
	 * Preprocess, Compile, and Run the current code. <P> There are three main
...
//  }
	...
	public String build(String buildPath, boolean verbose) throws RunnerException
	{
		...
		File[] files = buildFolder.listFiles();
		...
		// get the most recent updated source from the editor
		if (m_editor != null)
		{
			try
			{
				String source = m_editor.getText();

				getCode(0).setProgram(source);
			}
			catch(Exception e)
			{
			}
		...
		m_primaryClassName = preprocess(buildPath);

		// compile the program. errors will happen as a RunnerException
		// that will bubble up to whomever called build().
		Compiler compiler = new Compiler();
		
		if (compiler.compile(this, buildPath, m_primaryClassName, verbose))
		{
			size(compiler.getBuildPreferences());
			return m_primaryClassName;
		}
...
//	}
	...
	{		
		...
		{
			output(_("Low memory available, stability problems may occur"));
		}
	}
	...
	public boolean upload(String buildPath, String suggestedClassName, boolean usingProgrammer)
...
//			tc().ic.remove(this);
		...
		public int hashCode()
		{
			return m_sketch.getName().hashCode();
			//return tc().hashCode();
		}

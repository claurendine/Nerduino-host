 ...
 */
package com.nerduino.processing.app.debug;

import com.nerduino.processing.app.Base;
import com.nerduino.processing.app.I18n;
...
public class Compiler implements MessageConsumer
{
	...
	static final String SUPER_BADNESS =
			I18n.format("Compiler error, please submit this code to {0}", BUGS_URL);
	Sketch sketch;
	String buildPath;
	...
	int errors;
	ICompileCallback compileCallback;
						   ...
						   boolean verbose) throws RunnerException
	{
		...
		compileCallback.setStatus("");
		compileCallback.setProgress(0);		
		...
		compileCallback.setStatus("Preparing To Compile...");
		compileCallback.setProgress(20);		
		...
		compileCallback.setStatus("Compiling Sketch...");
		compileCallback.setProgress(30);
		...
		compileCallback.setStatus("Compiling Libraries...");
		compileCallback.setProgress(40);
		...
		compileCallback.setStatus("Compiling Core...");
		compileCallback.setProgress(50);
		...
		compileCallback.setStatus("Linking...");
		compileCallback.setProgress(60);
		...
		compileCallback.setStatus("Extracting EEPROM Data...");
		compileCallback.setProgress(70);
		...
		compileCallback.setStatus("Generating Hex File...");
		compileCallback.setProgress(80);
			...
			throws RunnerException
	{
		...
		for (File file : cSources)
		{
			...
			if (is_already_compiled(file, objectFile, dependFile, boardPreferences))
			{
				continue;
			}
	...
	private boolean is_already_compiled(File src, File obj, File dep, Map<String, String> prefs)
	{
		...
		{
			String str = "  Using previously compiled: " + obj.getPath();
	...
	private void execAsynchronously(List commandList) throws RunnerException
	{
		...
		if (result > 1)
		{
			...
			System.err.println(
					I18n.format("{0} returned {1}", command[0], result));
...
//    }
		...
		if (pieces != null)
		{
			...
			{
				error = "'Mouse' only supported on the Arduino Leonardo";
				//msg = "\nThe 'Mouse' class is only supported on the Arduino Leonardo.\n\n";
			}

			if (pieces[3].trim().equals("'Keyboard' was not declared in this scope"))
			{
				error = "'Keyboard' only supported on the Arduino Leonardo";
...
//        s = fileName + ":" + lineNum + ": error: " + pieces[3] + msg;        
		...
		if (s.indexOf("error:") > 0)
		{
			...
			compileCallback.setErrorCount(errors);
		}

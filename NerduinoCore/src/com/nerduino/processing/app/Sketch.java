...
import static com.nerduino.processing.app.I18n._;
import com.nerduino.processing.app.debug.AvrdudeUploader;
import com.nerduino.processing.app.debug.Compiler;
import com.nerduino.processing.app.debug.RunnerException;
import com.nerduino.processing.app.debug.Sizer;
import com.nerduino.processing.app.debug.Uploader;
import com.nerduino.processing.app.preproc.PdePreprocessor;
import com.nerduino.processing.core.*;
import java.awt.*;
 ...
 */
public class Sketch extends TreeNode implements IContext
{
	...
	ICompileCallback m_compileCallback;
	String m_sketchPath;
	...
	public void cloneSketch(Sketch m_selectedSketch)
	{
		...
		if (!f.exists() && f.mkdir())
		{
			...
			File[] list = this.getFolder().listFiles();
			...
			if (list != null)
			{
				for (File child : list)
				{
					...
					if (index > 0)
					{
						...
						if (rootname.equalsIgnoreCase(m_name))
						{
		...
		try
		{
			if (newpath != null && new File(newpath).exists())
			{
				...
				SketchManager.Current.addSketch(newsketch);
	...
	public void onSelected()
	{
		...
		if (BuilderTopComponent.Current != null)
			BuilderTopComponent.Current.setSketch(this);
	}

	public void compile(ICompileCallback callback)
	{
		try
		{
...
//          int result = JOptionPane.showOptionDialog(AppManager.Current.m_frame,
...
//  }
			...
			throws RunnerException
	{
		...
		if (compiler.compile(this, buildPath, m_primaryClassName, m_compileCallback, verbose))
		{
			size(buildPath, m_primaryClassName);
...
//    editor.status.progressUpdate(100);
			...
			throws RunnerException
	{
		...
		catch(RunnerException e)
		{
			...
			InputOutput io = IOProvider.getDefault().getIO("Build", false);
			io.getOut().println(I18n.format(_("Couldn't determine program size: {0}"), e.getMessage()));
		}
	...
	public String upload(String buildPath, String suggestedClassName, boolean usingProgrammer)

...
import com.nerduino.processing.app.debug.Target;
...
/**
 * The base class for the main processing application. Primary role of this
 * class is for platform identification and general interaction with the system
 * (launching URLs, loading files and images, etc) that comes from that.
 */
public class Base
{
...
//		}
	...
	/*
	 ...
	 protected void storeSketchPath(JEditPanel editor, int index)
	 {
	 String path = editor.getSketch().getMainFilePath();
	 String untitledPath = untitledFolder.getAbsolutePath();
	 if (path.startsWith(untitledPath))
	 ...
	 protected void handleActivated(JEditPanel whichEditor)
	 {
	 ...
	 JEditPanel lastOpened = editors.get(editors.size() - 1);
	 ...
	 if (path != null)
	 {
	 JEditPanel editor = handleOpen(path);
	 ...
	 public JEditPanel handleOpen(String path)
	 {
	 return handleOpen(path, nextEditorLocation());
	 }

	 protected JEditPanel handleOpen(String path, int[] location)
	 {
	 ...
	 JEditPanel editor = new JEditPanel(this, path, location);
	 //    Editor editor = null;
	 //    try {
	 ...
	 JEditPanel.serialMonitor.closeSerialPort();
	 storeSketches();
	 ...
	 rebuildSketchbookMenu(JEditPanel.sketchbookMenu);
	 rebuildToolbarMenu(JEditPanel.toolbarMenu);
	 ...
	 onBoardOrPortChange();
	 ArduinoSketch.buildSettingChanged();
	 }
	 };
	 ...
	 if (!ArduinoSketch.isSanitaryName(list[i]))
	 {
	 if (!builtOnce)
	 {
...
//      if (libraryJar.exists()) {
			//String sanityCheck = ArduinoSketch.sanitizeName(potentialName);
...
//          Compiler.packageListFromClassPath(libraryClassPath);
			...
			{
				if (subfile.isFile() && subfile.getName().endsWith(".h"))
				{
					importToLibraryTable.put(subfile.getName(), subfolder);
				}
			}
			/*
			String packages[] =
...
//      }
	...
	/*
	 ...
	 {
	 preferencesFrame = new Preferences();
	 }
	...
	{
		String target = Preferences.get("target");
		return Base.targetsTable.get(target);
	}
	...
	{
		String path = Preferences.get("sketchbook.path");

		if (path == null)
		{
			path = ArduinoManager.Current.getArduinoPath();
		}
	...
	static public void openURL(String url)
	{
		...
		catch(Exception e)
		{
			showWarning("Problem Opening URL",
						I18n.format("Could not open the URL\n{0}", url), e);
		}
	}
	...
	static public void openFolder(File file)
	{
		...
		catch(Exception e)
		{
			showWarning("Problem Opening Folder",
						I18n.format("Could not open the folder\n{0}", file.getAbsolutePath()), e);
		}
	}
	...
	static public File getContentFile(String name)
	{
		...
		String path = ArduinoManager.Current.getArduinoPath() + "/Source/Arduino";
...
//      <string>$JAVAROOT</string>
	...
	/**
	 * Read from a file with a bunch of attribute/value pairs that are separated
	 * by = and ignore comments with #.
	 */
	static public HashMap<String, String> readSettings(File inputFile)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (equals == -1)
			{
				...
				io.getOut().println("ignoring illegal line in " + inputFile);
	...
	static public void saveFile(String str, File file) throws IOException
	{
		...
		PApplet.saveStrings(temp, new String[]
				{
					str
				});
		if (file.exists())
		...
		{
			throw new IOException(
					I18n.format("Could not replace {0}",
								file.getAbsolutePath()));
		}
	}
	...
	static public void removeDir(File dir)
	{
		if (dir.exists())
		{
			...
			if (!dir.delete())
			{
				System.err.println(I18n.format("Could not delete {0}", dir));
			}
		}
	}
			...
			if (!dead.isDirectory())
			{
				if (!Preferences.getBoolean("compiler.save_build_files"))
				{
					if (!dead.delete())
					{
						...
						System.err.println(I18n.format("Could not delete {0}", dead));
					}
				}
			}
			else
	...
	/**
	 * Recursively creates a list of all files within the specified folder, and
	 * returns a list of their relative paths. Ignores any files/folders
	 * prefixed with a dot.
	 */

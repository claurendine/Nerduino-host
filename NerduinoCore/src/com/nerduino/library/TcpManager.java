...
public class XBeeManager implements FrameReceivedListener
{
	public static XBeeManager Current;
...
//        System.out.println("Frame Received!");
	...
	private void onGetMetaData(ZigbeeReceivePacketFrame zrf)
	{
		...
		short count = (short) NerduinoHost.Current.getHostPointCount();

		byte length = 15;
		byte[] data =
		{
			4, 'H', 'o', 's', 't', 4, 'H', 'o', 's', 't',
			NerduinoHost.Current.m_configurationToken,
			(byte) ((count >> 8) & 0xff),
			(byte) (count & 0xff),
			DeviceTypeEnum.DT_Host.Value(), NerduinoHost.Current.m_status
		};
